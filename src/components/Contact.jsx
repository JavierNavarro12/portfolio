...
function Contact({ isMobile }) {
  ...
  return (
    <section 
      id="contacto" 
      className="w-full flex flex-col items-center scroll-mt-24 main-section mt-10" 
      data-aos="fade-up" 
      data-aos-offset={isMobile ? "200" : "300"}
    >
      <div data-aos="fade-up">
        <h2 className="text-6xl md:text-7xl font-extrabold text-gray-900 dark:text-white mb-8 tracking-tight drop-shadow-lg transform transition-all duration-300 hover:scale-105 wiggle-on-hover">
          {t.contact}
        </h2>
      </div>
      <div className="flex flex-wrap justify-center gap-4 md:gap-8 mt-4 mb-16 w-full max-w-2xl">
        {contactLinks.map((link, index) => (
          ...
          >
            <div className={`absolute inset-0 bg-${link.color} rounded-full transform transition-transform duration-300 group-hover:scale-110`}></div>
            ...
            {link.icon}
            <span className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 text-sm font-medium text-gray-700 dark:text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              {link.label}
            </span>
          </a>
        ))}
