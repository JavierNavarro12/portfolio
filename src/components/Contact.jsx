import React, { useRef, useLayoutEffect } from 'react';
import { useLanguage } from '../contexts/LanguageContext';
import { gsap } from '../hooks/useGSAP';

function Contact() {
  const { t } = useLanguage();
  
  const sectionRef = useRef(null);
  const titleRef = useRef(null);
  const iconsRef = useRef([]);

  const contactLinks = [
    {
      href: "https://wa.me/34693744798",
      label: "WhatsApp",
      icon: (
        <svg className="w-8 h-8 text-green-500 relative z-10" fill="currentColor" viewBox="0 0 24 24">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
        </svg>
      ),
      color: "green-500",
      bgColor: "#22c55e"
    },
    {
      href: "https://github.com/JavierNavarro12",
      label: "GitHub",
      icon: (
        <svg className="w-8 h-8 text-gray-700 dark:text-gray-300 relative z-10" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
        </svg>
      ),
      color: "gray-700",
      bgColor: "#374151"
    },
    {
      href: "https://www.linkedin.com/in/javier-navarro-rodr%C3%ADguez-056023331/",
      label: "LinkedIn",
      icon: (
        <svg className="w-8 h-8 text-blue-600 relative z-10" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
        </svg>
      ),
      color: "blue-600",
      bgColor: "#2563eb"
    },
    {
      href: "mailto:navarrojavi107@gmail.com",
      label: "Gmail",
      icon: (
        <svg className="w-8 h-8 text-red-500 relative z-10" fill="currentColor" viewBox="0 0 24 24">
          <path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 2v.01L12 13 4 6.01V6h16zM4 20v-9.99l7.99 7.99c.39.39 1.02.39 1.41 0L20 10.01V20H4z" />
        </svg>
      ),
      color: "red-500",
      bgColor: "#ef4444"
    },
    {
      href: "/CV-Javier-Navarro.pdf",
      label: "Currículum",
      icon: (
        <svg className="w-8 h-8 text-blue-800 dark:text-blue-300 relative z-10" fill="currentColor" viewBox="0 0 24 24">
          <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6zM6 20V4h7v4h4v12H6z" />
        </svg>
      ),
      color: "blue-800",
      bgColor: "#1e40af"
    }
  ];

  useLayoutEffect(() => {
    if (!sectionRef.current) return;

    const ctx = gsap.context(() => {
      // Animación del título - simple fade in
      gsap.from(titleRef.current, {
        opacity: 0,
        y: 20,
        duration: 0.6,
        ease: 'power2.out',
        scrollTrigger: {
          trigger: sectionRef.current,
          start: 'top 75%',
          toggleActions: 'play none none reverse',
        },
      });

      // Animación de iconos - simple fade in con stagger
      iconsRef.current.forEach((icon, index) => {
        if (!icon) return;

        gsap.from(icon, {
          opacity: 0,
          y: 30,
          scale: 0.8,
          duration: 0.5,
          delay: index * 0.08,
          ease: 'power2.out',
          scrollTrigger: {
            trigger: sectionRef.current,
            start: 'top 70%',
            toggleActions: 'play none none reverse',
          },
        });
      });

    }, sectionRef);

    return () => ctx.revert();
  }, []);

  return (
    <section 
      id="contacto"
      ref={sectionRef}
      className="w-full flex flex-col items-center scroll-mt-24 main-section mt-10 relative"
    >
      <div style={{ perspective: '1000px' }}>
        <h2 
          ref={titleRef}
          className="text-6xl md:text-7xl font-extrabold text-gray-900 dark:text-white mb-8 tracking-tight drop-shadow-lg contact-title"
        >
          {t.contact}
        </h2>
      </div>
      
      <div className="flex flex-wrap justify-center gap-4 md:gap-8 mt-4 mb-16 w-full max-w-2xl">
        {contactLinks.map((link, index) => (
          <a
            key={link.label}
            ref={el => iconsRef.current[index] = el}
            href={link.href}
            target={link.href.startsWith('mailto:') || link.href.startsWith('/') ? '_self' : '_blank'}
            rel={link.href.startsWith('mailto:') || link.href.startsWith('/') ? '' : 'noopener noreferrer'}
            className="contact-icon group relative w-20 h-20 flex items-center justify-center transition-transform duration-200 hover:scale-110"
            aria-label={link.label}
          >
            <div 
              className="absolute inset-0 rounded-full transform transition-transform duration-300 group-hover:scale-110"
              style={{ backgroundColor: link.bgColor, opacity: 0.2 }}
            />
            <div className="absolute inset-0 bg-white dark:bg-gray-800 rounded-full transform transition-transform duration-300 group-hover:scale-90" />
            {link.icon}
            <span className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 text-sm font-medium text-gray-700 dark:text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap">
              {link.label}
            </span>
          </a>
        ))}
      </div>
    </section>
  );
}

export default Contact;
